...
- **Mitigation installed (2026-07-19):** `C:\Users\cborn\.openpnp2\scripts\Events\Job.Error.js` re-homes the machine (full mechanical + visual home via `machine.home()`) on every job error, before the error dialog is shown — a lost frame cannot survive into cancel/continue/resume. The `Job.Error` scripting event fires synchronously in `ReferencePnpJobProcessor.next()` when a step throws. Note: `Machine.AfterHoming.js` also runs after each of these re-homes (rotates N1/N2 to 0°, reselects top camera) — with a part still held on a nozzle it will be rotated to 0°; alignment afterwards should still handle it, but watch for surprises.
